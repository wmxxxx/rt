$(document).ready(function(){
	...
	function buildPage(dPanel){
			...
			if(dPanel.class == "p1"){
				$("#tabContent1 .layTop").html(
					'<div class="airPanel" style="transform:scale('+ratio+')">'
						...
						+'</div>'
						+'<div class="aLeft"></div>'
						+'<div class="aRight"><div><i class="icon icon-star"></i><p class="airtype">--</p></div><div><i class="icon icon-bolt"></i><p class="airenergy">--</p></div><div><i class="icon icon-time"></i><p class="airtime">--</p></div></div>'
					+'</div>'
				);
		...
		$.each(dInfo.data,function(i,k){
			...
			if(i == "TotalTime")$("#tabContent1 .aRight .airtime").html(k.show+k.unit);
		});
		// 外接设备
		...
		$.post("php/getRunning.php",{
			...
			lead.highcharts({
				...
				xAxis:{
					...
					plotBands:[{
						color:"#faaf3b80",
						from:new Date("2011-11-11 "+result.time.tran_start_time1+":00:00").getTime(),
						to:new Date("2011-11-11 "+result.time.tran_end_time1+":00:00").getTime(),
						zIndex:3,
						label:{
							text:"过渡时间",
							y:-5
						...
						color:"#00B9B480",
						from:new Date("2011-11-11 "+result.time.work_start_time+":00:00").getTime(),
						to:new Date("2011-11-11 "+result.time.work_end_time+":00:00").getTime(),
						zIndex:3,
						label:{
							text:"工作时间",
							y:-5
						...
						color:"#faaf3b80",
						from:new Date("2011-11-11 "+result.time.tran_start_time2+":00:00").getTime(),
						to:new Date("2011-11-11 "+result.time.tran_end_time2+":00:00").getTime(),
						zIndex:3,
						label:{
							text:"过渡时间",
							y:-5
