	...
	function task_TempHum($app,$fun){
		...
		foreach($data as $d){
			...
			if($d -> access[0] -> tpl_tag == "city"){
				...
				$outTemp = $d -> access[0] -> variantDatas[0] -> value;
				$outHum = $d -> access[0] -> variantDatas[1] -> value;
			}
			if($d -> tpl_tag == "fjpg2"){
				$temp = $d -> data["RoomTemp"] -> value;
				$hum = $d -> data["RoomHumi"] -> value;
				$open = $d -> data["RelayStatus"] -> value;
			}else if($d -> tpl_tag == "hvac2"){
				$temp = $d -> data["RoomTemp"] -> value;
				$open = $d -> data["StartStopStatus"] -> value;
			}else if($d -> tpl_tag == "ftkt2"){
				$temp = $d -> data["RoomTemp"] -> value;
				$hum = $d -> data["RoomHum"] -> value;
				$open = $d -> data["StartStopStatus"] -> value;
			}
			if(isset($d -> data["Ep"])){
