            ...
            try {
                if (sys_type == "1" && (fun.F_IsConfig || fun.F_EnergyConfig || fun.F_TemplateConfig || fun.F_DeviceConfig)) {
                            ...
                            success: function (resObj) {
                                ...
                                for (var i = 0; i < resObj.length; i++) {
                                    szNodes.push({
                                        ...
                                        icon: "/Resources/images/folder.png",
                                        iconOpen: "/Resources/images/folder_open.png",
                                        iconClose: "/Resources/images/folder.png"
                                    });
                                }
                                $.fn.zTree.init($("#configPanel"), setting, szNodes);
                            }
                    ...
                    if (fun.F_GuideMode == 'js') {
                        eval('TUI.env.currPlugin = new ' + fun.F_PluginTag + '("middlePanel",{ code:' + fun.F_PluginCode + ',fun:' + fun.F_FunctionCode + ',app:' + sys_no + ',tree:' + (fun.F_TreeNo == '' ? '""' : fun.F_TreeNo) + ', user:' + TUI.env.us.code + ',env:' + (fun.F_EnvVar == '' ? '""' : fun.F_EnvVar) + '})');
                        TUI.env.currPlugin.init();
                    ...
                    if (fun.F_GuideMode == 'js') {
                        eval('TUI.env.currPlugin = new ' + fun.F_PluginTag + '("centerPanel",{ code:' + fun.F_PluginCode + ',fun:' + fun.F_FunctionCode + ',app:' + sys_no + ',tree:' + (fun.F_TreeNo == '' ? '""' : fun.F_TreeNo) + ', user:' + TUI.env.us.code + ',env:' + (fun.F_EnvVar == '' ? '""' : fun.F_EnvVar) + '})');
                        TUI.env.currPlugin.init();
                    } else if (fun.F_GuideMode == 'html') {
                        ...
                        success: function (result) {
                            ...
                            for (var i = 0; i < result.length; i++) {
                                szNodes.push({
                                    ...
                                    icon: result[i].otype == "1" || result[i].otype == "" ? "/Resources/images/folder.png" : (result[i].otype == "2" ? "/Resources/images/node.png" : "/Resources/images/facility.png")
                                });
                            }
                    ...
                    if (resObj.length > 0) {
                        ...
                        for (var i = 0; i < resObj.length; i++) {
                            szNodes.push({
                                ...
                                open: true,
                                checked: resObj[i].checked == 1 ? true : false,
                                icon: "/Resources/images/tree/" + (resObj[i].type == '1' || resObj[i].type == null ? 'folder' : (resObj[i].av == 1 ? 'meter_green' : 'meter_virtual')) + ".png",
                                iconOpen: "/Resources/images/tree/" + (resObj[i].type == '1' || resObj[i].type == null ? 'folder_open' : (resObj[i].av == 1 ? 'meter_green' : 'meter_virtual')) + ".png",
                                iconClose: "/Resources/images/tree/" + (resObj[i].type == '1' || resObj[i].type == null ? 'folder' : (resObj[i].av == 1 ? 'meter_green' : 'meter_virtual')) + ".png"
                            });
                        }
                        ...
                        if (result.length > 0) {
                            ...
                            if (result[0].F_PluginTag != null && result[0].F_PluginTag != '') {
                                if (result[0].F_GuideMode == 'js') {
                                    eval('TUI.env.currPlugin = new ' + result[0].F_PluginTag + '("centerPanel",{ code:' + result[0].F_PluginCode + ',fun:' + result[0].F_FunctionCode + ',data:' + escape(JSON.stringify(data)) + ',app:' + sys_no + ',tree:' + (result[0].F_TreeNo == '' ? '""' : result[0].F_TreeNo) + ', user:' + TUI.env.us.code + ',env:' + (result[0].F_EnvVar == '' ? '""' : result[0].F_EnvVar) + '})');
                                    TUI.env.currPlugin.init();
