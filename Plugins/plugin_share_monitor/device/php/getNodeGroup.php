<?php
	...
	$result = array(
		...
		"strategy" => $str,
		"model" => $db -> query("select distinct c.* from [Things+].[dbo].[TB_Share_GroupToNode] a,[Things+].[dbo].[TB_Share_Group] b,[Things+].[dbo].[TB_Share_Model] c where a.F_group_id = b.F_id and b.F_project_code = ".$_POST["app"]." and a.F_node_id = ".$_POST["id"]." and c.F_id like '%flex%'"),
		"force" => $db -> query("select * from [Things+].[dbo].[TB_Share_Group] a,[Things+].[dbo].[TB_Share_GroupToNode] b where a.F_id = b.F_group_id and a.F_id like '%force%' and a.F_project_code = ".$_POST["app"]." and b.F_node_id = ".$_POST["id"])
	);
	ob_clean();
