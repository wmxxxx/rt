<?php
    ...
    try
    {
        ...
        if(!$redis -> get("formula_map")){
            $formula_map = array();
            $sql = "select a.F_NodeCode as ncode,a.F_NodeName as nname,a.F_ValueCode as vcode,a.F_ValueLabel as vlabel,a.F_ValueName as vname,a.F_PrecisionRatio as pr,a.F_DecimalPoint as dp,a.F_Formula as formula,a.F_KV as kv,a.F_IsStorage as storage,a.F_StorageCycle as scycle,a.F_NodeCode as  map_ncode,b.F_ValueCode as map_vcode,b.F_ValueLabel as map_vlabel,'tpl' as map_type from (select a.F_NodeCode,a.F_NodeName,b.F_ValueCode,b.F_ValueName,b.F_ValueLabel,b.F_PrecisionRatio,b.F_DecimalPoint,b.F_Formula,b.F_KV,b.F_IsStorage,b.F_StorageCycle from dbo.tb_A_IoTNode a,dbo.tb_A_Value b where b.F_Formula <> '' and b.F_IsRefer = 0 and a.F_TemplateCode = b.F_TemplateCode) a,(select a.F_NodeCode,b.F_ValueCode,b.F_ValueLabel from dbo.tb_A_IoTNode a,dbo.tb_A_Value b where a.F_TemplateCode = b.F_TemplateCode) b where a.F_NodeCode = b.F_NodeCode and b.F_ValueLabel = a.F_Formula union select a.F_VirtualNCode as ncode,b.F_EntityName as nname,a.F_VirtualVCode as vcode,a.F_VirtualVLabel as vlabel,c.F_ValueName as vname,c.F_PrecisionRatio as pr,c.F_DecimalPoint as dp,a.F_Formula as formula,c.F_KV as kv,c.F_IsStorage as storage,c.F_StorageCycle as scycle,d.F_EntityID as map_ncode,e.F_ValueCode as map_vcode,e.F_ValueLabel as map_vlabel,'node' as map_type from dbo.tb_B_VirtualCompute a,dbo.tb_B_EntityTreeModel b,dbo.tb_A_Value c,dbo.tb_B_EntityTreeModel d,dbo.tb_A_Value e where a.F_MType = '1' and a.F_VirtualNCode = b.F_EntityID and a.F_VirtualVCode = c.F_ValueCode and d.F_EntityID = substring(a.F_NAndV,1,CHARINDEX('.',a.F_NAndV) - 1) and d.F_NodeTemplate = e.F_TemplateCode and e.F_ValueLabel = substring(a.F_NAndV,CHARINDEX('.',a.F_NAndV) + 1,len(a.F_NAndV))";
            $result = $db -> query($sql);
            for($i = 0;$i < count($result);$i++){
        ...
        if(!$redis -> get("param_deviation")){
		    ...
		    $redis -> set("param_deviation",json_encode($param_deviation));
    ...
    try
    {
        ...
        $sql = "select top 10000 F_NodeCode as ncode,F_ValueCode as vcode ,F_ValueLabel as vlabel,convert(varchar,F_StartTime,120) as startTime,convert(varchar,F_EndTime,120) as endTime from tb_C_RepairDataTotal where F_RepairStatus = 0 order by F_StartTime desc";
        $repair = $db -> query($sql);
        echo '[' . date("Y-m-d H:i:s",time()) . ']：系统服务读取最新待断点续传记录' . count($repair) . '条！<br>';
        for($i = 0;$i < count($repair);$i++){
            $status = false;
            $ncode = $repair[$i] -> ncode;
            $vcode = $repair[$i] -> vcode;
            ...
            for($k = 0;$k < count($resJson -> data);$k++){
                ...
                $end_value = $value + (array_key_exists($node_code_label,$param_deviation_cache) ? $param_deviation_cache -> $node_code_label -> dvalue : 0);
                ...
                if($time - $storagetime_stamp > $param_obj -> scycle){
					...
					//echo '<br>';
                    if($valid){
                        $sql = "insert into tb_C_DataBuffer values (" . $ncode . "," . $vcode . ",'" . $vlabel . "','" . date("Y-m-d H:i:s",$time) . "'," . $end_value . ",1) insert into tb_C_RepairDataDetail values (" . $ncode . "," . $vcode . ",'" . $vlabel . "','" . date("Y-m-d H:i:s",$time) . "'," . $end_value . ",getdate()) ";
						//echo $sql;
                        if($db -> execute($sql)){
                            echo '[' . date("Y-m-d H:i:s",time()) . ']：' . '系统服务续传设备“' . $node_cache -> $ncode -> name . '”参数“' . $vlabel . '”[' . date("Y-m-d H:i:s",$time) . ']点的数据值' . $end_value . '到数据库成功！<br>';
                        }
                        if (property_exists($formula_map_cache,$node_code_label)){
							$tempArray = $formula_map_cache -> $node_code_label;
							$formula_map = $tempArray[0];
							if($formula_map -> storage != 1){
								continue;
							}
                            $m_ncode = $formula_map -> ncode;
                            $m_vcode = $formula_map -> vcode;
                            $m_vlabel = $formula_map -> vlabel;
                            if($formula_map -> map_type == 'tpl'){
                                $m_value = number_format($formula_map -> pr * str_replace($vlabel,$end_value,$formula_map -> formula),$formula_map -> dp, ".", "");
								echo 'start xuni value';
								echo '<br>';
                            ...
                            $sql = "insert into tb_C_DataBuffer values (" . $m_ncode . "," . $m_vcode . ",'" . $m_vlabel . "','" . date("Y-m-d H:i:s",$time) . "'," . $m_value . ",1) ";
							//echo $sql;
                            if($db -> execute($sql)){
            ...
            while($totalCount - $tempCount > 0){
                ...
                for($k = 0;$k < count($resJson2 -> data);$k++){
                    ...
                    $end_value = $value + (property_exists($node_code_label,$param_deviation_cache) ? $param_deviation_cache -> $node_code_label -> dvalue : 0);
                    ...
                    if($time - $storagetime_stamp > $param_obj -> scycle){
                        if($valid){
                            ...
                            if (property_exists($node_code_label,$formula_map_cache) && $formula_map_cache -> $node_code_label -> storage == 1){
                                $m_ncode = $formula_map_cache -> $node_code_label -> ncode;
                                $m_vcode = $formula_map_cache -> $node_code_label -> vcode;
            ...
            if($status){
                $sql = "update tb_C_RepairDataTotal set F_RepairStatus = 1,F_RepairTime = getdate() where F_NodeCode = " . $ncode . " and F_ValueCode = " . $vcode . " and F_StartTime = '" . $repair[$i] -> startTime . "'";
                if($db -> execute($sql)){
