<?php
    ...
    $sql = "select a.F_PropertyID,a.F_PropertyIdentifier,a.F_PropertyName from dbo.tb_B_ObjectProperty a,dbo.tb_B_DictTreeProperty b where b.F_GroupID = " . $device . " and b.F_PropertyID = a.F_PropertyID and a.F_PropertyTypeID <> '4' order by b.F_OrderNum";
	$result = $db -> query($sql);
    ...
    $sql = "exec proc_A_ExportNodeInfo null,null,1," . $device;
	$result = $db -> query($sql);
    $device_name = '';
    foreach ($result as $obj){
        if($device_name == '') $device_name = $obj -> F_DeviceTypeName;
    ...
    $filename = urlencode($device_name . '-设备配置关系模板（' . date('Y-m-d-His') . '）.csv');
